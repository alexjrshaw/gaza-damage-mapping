...
EXPECTED = {
    "retrained_t0.5":  {"f1": 0.363, "precision": 0.931, "recall": 0.226, "roc_auc": 0.608, "accuracy": 0.718},
    "retrained_t0.67": {"f1": 0.181, "precision": 0.973, "recall": 0.100, "roc_auc": 0.549, "accuracy": 0.678},
}
...
result = subprocess.run(
    ["python3", "src/data/transfer_cities/pixel_inference/evaluate_pixel_transfer.py", "--city", "MOS_RETRAINED_EAST_ONLY"],
    cwd="/scratch/s1214882/gaza-damage-mapping", capture_output=True, text=True
)
...
y_true = np.concatenate([np.ones(pos_vals2[~np.isnan(pos_vals2)].size), np.zeros(neg_vals2[~np.isnan(neg_vals2)].size)])
...
if optimal_t is not None:
    ...
    all_ok &= check("Retrained own-optimal (fresh)", actual_opt,
                     {"f1": 0.451, "precision": 0.901, "recall": 0.301, "roc_auc": 0.641, "accuracy": 0.7386})

print("\n" + "="*70)
print(f"OVERALL: {'ALL FIGURES VERIFIED CORRECT' if all_ok else 'SOME FIGURES DO NOT MATCH - investigate'}")
print("="*70)