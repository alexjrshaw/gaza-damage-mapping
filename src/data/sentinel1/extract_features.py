...
def create_dataset_ready_all_dates(
    ...
    Args:
        ...
        d_periods (dict[str, list[tuple[str]]): The time periods to use. The keys are "pre" and "post" and the values
            are a list of date tuples (start, end).
    ...
    for pre in d_periods["pre"]:
        for post in d_periods["post"]:
            ...
            fs += create_dataset(aois, damages_to_keep, d_periods_, extract_window, reducer_names)
    ...
    if export:
        ...
        ee.batch.Export.table.toAsset(
            ...
            assetId=ASSETS_PATH + f"features_ready/s1_{extract_window}_{period}_{split}",
        ).start()
...
def create_dataset(
    ...
    for aoi, orbit in aoi_orbit_iterator():
        ...
        fc = get_fc_ts(aoi, orbit, extract_window, damages_to_keep)
...
def get_fc_ts(aoi: str, orbit: int, extract: str, damages_to_keep: list[int] | None = [1, 2]) -> ee.FeatureCollection:
    """Load precomputed features for training or testing given the aoi and orbit."""

    fc_path = ASSETS_PATH + f"intermediate_features/ts_s1_{extract}/{aoi}_orbit{orbit}"
