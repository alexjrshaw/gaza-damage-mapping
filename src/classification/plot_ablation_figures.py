...
# Baselines from evaluation.ipynb pixel-level run at t=0.655
PIXEL_BASELINE_F1  = 0.884
PIXEL_BASELINE_AUC = 0.741

# Point-level baseline (VV+VH, all 7 reducers, n=50 trees) at t=0.655
POINT_BASELINE_F1  = 0.665  # from ablation_results.json n_trees["50"]["t0.655"]["f1"]
...
def plot_oob_n_trees(rp: dict, rx: dict) -> None:
    ...
    ax.plot(pt["tree_counts"], pt["oob_errors"],
    ...
    if "oob_n_trees" in rx:
        px = rx["oob_n_trees"]
        ax.plot(px["n_trees"], px["oob_error"],
                color=ORANGE, linewidth=2, marker="s", markersize=4,
                linestyle="--", label="Gaza (pixel-level, this study)")
...
def plot_oob_mtry(rp: dict, rx: dict, mtry_ext: dict) -> None:
    ...
    ax.plot(ext_x, ext_y, color=GREY, linewidth=1.5, linestyle="--",
            label="Gaza point-level (extended sweep)", alpha=0.7)

    # Main point-level sweep
    ax.plot(pt["mtry_values"], pt["oob_errors"],
            color=BLUE, linewidth=2, marker="o", markersize=5,
            label="Gaza point-level")

    # Pixel-level OOB if available
    if "oob_mtry" in rx:
        px = rx["oob_mtry"]
        ax.plot(px["mtry"], px["oob_error"],
                color=ORANGE, linewidth=2, marker="s", markersize=5,
                linestyle="--", label="Gaza pixel-level")
...
def plot_f1_n_trees(rp: dict, rx: dict) -> None:
    ...
    px_map = {10: "ablation_ntrees_10", 25: "ablation_ntrees_25",
              50: None, 100: "ablation_ntrees_100",
              200: "ablation_ntrees_200", 300: "ablation_ntrees_300"}
