...
GOVERNORATE_ALIASES = {
    ...
    "deir al-balah": "deir al-balah", "deir al balah": "deir al-balah",
    "north gaza": "north gaza", "gaza": "gaza", "rafah": "rafah",
}
...
PRE_WINDOW_COLS = [
    "2021-10-07","2022-10-07","2022-12-07","2023-02-07",
    "2023-04-07","2023-06-07","2023-08-07",
]
POST_WINDOW_COLS = [
    "2023-10-07","2023-12-07","2024-02-07","2024-04-07",
    "2024-06-07","2024-08-07","2024-10-07","2024-12-07",
    "2025-02-07","2025-04-07","2025-06-07","2025-08-07","2025-10-07",
]
PANEL_LABELS = [
...
def main():
    ...
    name_col   = [c for c in admin2_raw.columns
                  if "name" in c.lower() and "ar" not in c.lower()][0]
    gov_names  = set(gdf["adm2_name"].apply(normalize_governorate_name))
    admin2     = admin2_raw[admin2_raw[name_col].apply(
                     normalize_governorate_name).isin(gov_names)]
    ...
    for i, label in enumerate(PANEL_LABELS):
        ...
        ax.scatter(x_rot[has_old], y_rot[has_old],
                   s=MARKER_SIZE, color=COLOR_OLD, linewidths=0)
        ax.scatter(x_rot[has_new], y_rot[has_new],
                   s=MARKER_SIZE, color=COLOR_NEW, linewidths=0)
    ...
    legend_ax.legend(
        handles=[
            Line2D([0], [0], marker="o", color="w", markerfacecolor=COLOR_OLD,
                   markersize=10, label="Old damage"),
            Line2D([0], [0], marker="o", color="w", markerfacecolor=COLOR_NEW,
                   markersize=10, label="New damage"),
        ],
        loc="center", fontsize=10, frameon=False,
    )

    fig.suptitle("Gaza: cumulative building damage by assessment window",
                 fontsize=13, y=0.99)
